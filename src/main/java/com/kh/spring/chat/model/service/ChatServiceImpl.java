...
public class ChatServiceImpl implements ChatService{
	...
	public int openChatRoom(ChatRoom room) {
		...
		// transactional 어노테이션은 DML문이 한 세션에 여러개일 때 사용
	}
	...
	public List<ChatMessage> joinChatRoom(ChatRoomJoin join) {
		...
		int result = dao.joinCheck(join); // 참여하고있다면 1, 없다면 0
		if (result == 0) {
	...
	public void exitChatRoom(ChatMessage chatMessage) {
		...
		if (result == 0) throw new RuntimeException("채팅방 나가기 처리 에러");
		...
		if (cnt == 0) {
			...
			if (result == 0) throw new RuntimeException("채팅방 삭제 오류");
		}
	}
